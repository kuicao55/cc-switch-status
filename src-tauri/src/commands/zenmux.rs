...
/// PAYG Balance from ZenMux API
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PaygBalance {
    pub currency: String,
...
#[tauri::command]
pub async fn fetch_zenmux_payg_balance(
    api_key: String,
) -> Result<PaygBalance, AppError> {
    let client = reqwest::Client::new();
    ...
    Ok(PaygBalance {
        ...
        total_credits: data.get("total_credits")
            .and_then(|v| v.as_f64()).unwrap_or(0.0),
        top_up_credits: data.get("top_up_credits")
            .and_then(|v| v.as_f64()).unwrap_or(0.0),
        bonus_credits: data.get("bonus_credits")
            .and_then(|v| v.as_f64()).unwrap_or(0.0),
    })
}
