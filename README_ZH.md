...
**CC Switch** 为你提供一个桌面应用来管理所有五个 CLI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 50+ 供应商预设、统一的 MCP, SKILLS 管理以及系统托盘弹窗快速切换和用量查看功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。
...
- **系统托盘快速切换** — 点击托盘图标打开自定义弹窗，查看应用标签、切换供应商并查看用量统计
...
2. **切换供应商**：
   - 主界面：选择供应商 → 点击"启用"
   - 系统托盘：点击托盘图标打开弹窗，然后点击供应商名称即可立即切换
